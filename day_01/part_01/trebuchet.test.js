...
describe("Trebuchet Part 01", () => {
  test("Should return right value from initial_sample.txt provided by AOC", () => {
    ...
    expect(discoverCalibration(filePath)).toBe(142);
  });

  test("Should return right value from puzzle_input.txt", () => {
    const filePath = path.join(__dirname, "puzzle_input.txt provided by AOC");

    console.log(discoverCalibration(filePath));

    expect(discoverCalibration(filePath)).toBe(54304);
  });
});
